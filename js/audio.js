...
export class WeddingAudio {
 constructor(){
  ...
  this.media.autoplay=Boolean(CONFIG.music.autoplay);this.media.loop=true;this.media.preload='auto';this.media.volume=CONFIG.music.volume;
 ...
 async start(){
  ...
  this.pendingPlay=this.media.play().then(()=>{if(!this.enabled||document.hidden)this.media.pause();this.onStateChange();return true;}).catch(error=>{
   ...
   this.onStateChange();return false;
